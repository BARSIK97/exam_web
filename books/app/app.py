...
def load_user(user_id):
    ...
    if users is not None:
        return User(user_id=users.user_id, user_login=users.login, role_id=users.role_id, first_name=users.first_name, middle_name=users.middle_name, last_name=users.last_name)
    return None

class User(UserMixin):
    def __init__(self, user_id, user_login, role_id, first_name, middle_name, last_name):
        self.id = user_id
...
def auth():
    ...
    if request.method == 'POST':
        ...
        if users is not None:
            ...
            login_user(User(user_id=users.user_id, user_login=users.login, role_id=users.role_id, 
                            first_name=users.first_name, middle_name=users.middle_name, last_name=users.last_name), remember=remember_me)
...
def index():
    ...
    with db_connector.connect().cursor(named_tuple=True) as cursor:
        ...
        page_count = (record_count // MAX_PER_PAGE) + (1 if record_count % MAX_PER_PAGE > 0 else 0)
        pages = range(max(1, page - 3), min(page_count, page + 3) + 1)

    return render_template('index.html', books=books, page=page, pages=pages, page_count=page_count)
...
def new():
    ...
    if request.method == 'POST':
        ...
        if cleaned_descr is None:
            return render_template('new.html', book_data=book_data, all_genres=all_genres, selected_genres=selected_genres, errors=errors)       
        book_data = {
...
def view(book_id):
    ...
    return render_template('view.html', book_data=book_data, genres=genres, reviews=reviews, user_review=user_review)
...
def edit(book_id):
    ...
    if request.method == 'POST':
        cleaned_descr = clean_content(request.form.get('book_description').strip())
        if cleaned_descr is None:
            return render_template('edit.html', book_data=book_data, all_genres=all_genres, selected_genres=selected_genres, errors=errors)
        book_data = {
            'book_name': request.form['book_name'],
            'book_description': request.form['book_description'],
            'year': request.form['year'],
